...
│   ├── single      Analyze a single trace directory
│   ├── sweep       Analyze a multi-config sweep directory
│   └── gemm        Extract top GEMM kernels by variance
├── process     Data processing utilities
│   ├── gpu-timeline    Aggregate GPU timeline across ranks
│   ├── comms           Process NCCL collective data
│   └── gemm-variance   Enhance GEMM variance with timestamps
├── compare     Compare baseline vs test reports
│   ├── gpu_timeline    Compare GPU timeline Excel reports
│   └── collective      Compare NCCL collective Excel reports
├── generate    Generate reports and visualizations
│   ├── excel       Comprehensive Excel report with formatting
│   ├── plots       PNG visualization plots
│   └── html        Self-contained HTML dashboard
└── pipeline    End-to-end orchestration
    ├── summary     Full A/B comparison pipeline (or single-config)
    └── gemm        GEMM kernel variance analysis pipeline
```

The `pipeline` commands are the recommended entry point — they orchestrate the other command groups automatically.

### Input Directory Structure

**A/B Comparison (e.g., RCCL Warp Speed):**
...
├── 256thread/
│   ├── nccl_28channels/
│   │   └── torch_profiler/rank*/trace/pt.trace.json
│   └── nccl_56channels/
│       └── torch_profiler/rank*/trace/pt.trace.json
├── 512thread/
│   ├── nccl_28channels/...
...
```bash
# From the aorta repository root
cd /path/to/aorta
...
Options:
  --skip-tracelens                Skip TraceLens analysis, only aggregate existing reports
...
Options:
  -b, --baseline PATH    Baseline trace directory (optional; omit for single-config mode)
...
Options:
  --sweep-dir PATH                Sweep directory with thread/channel subdirectories (required)
  -o, --output PATH               Output directory (required)
  --skip-tracelens                Skip TraceLens analysis, use existing reports
  --short-kernel-threshold INT    Threshold for short kernel study (default: 50 µs)
  --topk-ops INT                  Number of top operations to include (default: 100)
  --top-k INT                     Number of top kernels to extract (default: 5)
