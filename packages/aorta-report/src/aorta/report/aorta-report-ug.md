...
This generates:
- GEMM variance CSV analysis
- Variance visualization plots (5 files)
...
|------|---------|------------------|
| `sweep` | Compare GEMM variance between two experiment sweeps (side-by-side) | `--sweep1`, `--sweep2` |
| `performance` | GPU/NCCL performance analysis report | `--plots-dir` |

**Examples:**

```bash
# Sweep comparison (GEMM variance between two sweeps)
...
> **Note on Two-Sweep Comparison:**
> - The `--mode sweep` option generates a **side-by-side comparison** of GEMM kernel variance plots from two different experiment sweeps.
> - Each sweep directory must contain `plots/` with the 5 GEMM variance plots (generated by `generate plots --type gemm` or `pipeline gemm`).
> - This two-sweep comparison is **NOT automatically handled** by `pipeline gemm` (which only analyzes a single sweep).
> - For baseline vs test comparisons, you need to:
>   1. Run `pipeline gemm` on each sweep separately
>   2. Then run `generate html --mode sweep` to create the comparison report
...
> **Automatic Comparison:** Unlike `pipeline gemm` (single sweep), `pipeline summary` **automatically handles the comparison** between baseline and test directories in a single command. All comparison reports (GPU timeline, collective/NCCL, Excel, plots, HTML) are generated with both baseline and test data combined and compared.
...
> **Important: Single Sweep Only**
> - `pipeline gemm` analyzes a **single sweep directory** and generates its variance analysis.
> - To compare two sweeps (baseline vs test), you must:
>   1. Run `pipeline gemm` on each sweep separately
>   2. Use `generate html --mode sweep` to create a side-by-side comparison
>
> ```bash
> # Example: Comparing two GEMM sweeps
> aorta-report pipeline gemm --sweep-dir /path/to/baseline_sweep -o ./baseline_output
> aorta-report pipeline gemm --sweep-dir /path/to/test_sweep -o ./test_output
> aorta-report generate html --mode sweep \
>     --sweep1 ./baseline_output --sweep2 ./test_output \
>     --label1 "Baseline" --label2 "Test" \
...
```bash
# Full pipeline (generates CSV, plots, and HTML report)
aorta-report pipeline gemm \
    --sweep-dir /path/to/sweep \
    -o /path/to/output \
    --top-k 10
...
# Note: HTML report is auto-generated by the pipeline; for manual generation,
# it's integrated into the pipeline and not exposed as a separate command.
```
...
├── gemm_variance_report.html                    # Self-contained HTML report with embedded plots
└── plots/
    ├── variance_by_threads_boxplot.png
    ├── variance_by_channels_boxplot.png
    ├── variance_by_ranks_boxplot.png
