...
| --- | --- | --- | --- |
| `schema_version` | `str` | constant | Currently `"1.9"`. See the changelog comment in `src/aorta/instrumentation/environment.py` next to the `SCHEMA_VERSION` constant for the field-by-field history. (1.8 is reserved for PR #226, expected to land first.) |
...
| `rocfft_catalog` | `dict` | optional `rocfft_kernel_cache.db` under `$ROCFFT_RTC_CACHE_PATH` or `/opt/rocm/lib/` | Fingerprint of rocFFT's **optional** AOT kernel cache. rocFFT usually compiles at runtime, so absence is normal: a *probed* "no cache" result is `status:"absent"` with **no** partial reason. (The default/backfill/disaster shape is `status:"partial"` with a "not captured" reason instead, so a snapshot that never probed is distinguishable from one that probed and found nothing.) `doc` + `status` (`ok`/`absent`/`partial`) + `env_overrides` (the `ROCFFT_RTC_CACHE_PATH` value, recorded even when no cache is found so a configured-but-empty override is visible) + `kernel_cache` (`present`, `path`, `source`, `size`, `sha256`, `reason`). Added in schema 1.9 (issue #54 follow-up). |
...
> **Numbering note:** 1.8 is reserved for PR #226 (env-probe Buck-target
...
> in-flight env-probe PRs stay monotonic; renumber on rebase if the
> merge order flips.
...
* New top-level **`rocfft_catalog`** (issue #54 follow-up): fingerprints
  ...
  is the common case (`status:"absent"`, no partial reason).
...
| `rocfft_catalog.kernel_cache.{present,path,source,size,sha256}` | the optional `rocfft_kernel_cache.db` found under `$ROCFFT_RTC_CACHE_PATH` (`"ROCFFT_RTC_CACHE_PATH"`) or `/opt/rocm/lib/` (`"rocm_lib"`); all `null` + `status:"absent"` when no cache exists (the common case) |
| `rocfft_catalog.env_overrides.ROCFFT_RTC_CACHE_PATH` | the raw `$ROCFFT_RTC_CACHE_PATH` value (or `null`), recorded even when no cache file is found so a configured-but-empty override is diffable |
